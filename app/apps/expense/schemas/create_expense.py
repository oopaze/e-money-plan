...
class CreateExpenseSchema(Schema):
    ...
    name = fields.String(required=True, allow_none=False, error_messages=default_error_messages)
    color = fields.String(required=True, allow_none=False, error_messages=default_error_messages)
    due_date = fields.Date("%d/%m/%Y", required=True, error_messages=default_error_messages)
